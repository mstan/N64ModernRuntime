// audio_shadow.cpp — see librecomp/audio_shadow.hpp.
//
...
//
// Algorithm is byte-for-byte the gbarecomp C++ port; only the namespace
// (gba:: -> recomp::) and the enum name (Judgement -> ShadowJudgement) differ
// to fit this runtime's conventions and avoid collision with the libultra-
// adjacent symbols. No numerical constants or control flow changed.

#include "librecomp/audio_shadow.hpp"
...
ShadowJudgement ShadowVerifier::judge(float canon_l, float canon_r,
                                      float chk_l, float chk_r) {
    float r, ratio;
    if (!check_.push(canon_l, canon_r, chk_l, chk_r, r, ratio))
        return ShadowJudgement::None;
    last_r_ = r;
    last_ratio_ = ratio;
    ...
    return window_ok ? ShadowJudgement::Pass : ShadowJudgement::Fail;
}
